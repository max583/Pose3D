...
2. Implement the hip-first solver:
   ...
   - next, model the knee joint, including hinge-like flexion, patella/anterior direction, and very limited tibia axial twist;
     ...
     - **slice 2 blocked on user decision** (2026-05-09): the substitution `constrainKneeFlexionWithFixedThigh → solveKneePose` is **not** a pure refactor. The old function allows `patella backward` (signed flexion negative range) in high-front hip poses; `legKnee.ts` forbids it (`patellaAngle ∈ [−20°, +45°]`). This affects ankle-drag UX through "ankle at hip level" intermediate states. Three options documented in `ai/tasks/leg-hierarchical-solver-design.md` § «Slice 2 Substitution — Semantic Analysis»: **A** anatomical purity (drop old flexibility, ankle drag may snap at limits), **B** pragmatic shim (`allowPatellaBackwardInHighFront` flag), **C** knee-target driven control (slice 3 of original plan). Substitution waits on user pick; legKnee.ts is staged.
     - manual reference-pose verification (raised 150° kneeFlexion limit) — independent of A/B/C choice, can be done now in the viewport;
