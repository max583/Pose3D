...
3. **(Promoted to next code step 2026-05-09 — slice 3 of knee-layer plan.)** Add an additional leg-position control path through the knee node. Keep the current ankle-driven control because it is generally natural, but do not make it the only way to position the leg. Now blocks the slice 4 ankle-IK substitution: knee-node controller must ship first so the user has a direct tool for extreme tucked poses before the patella-backward allowance is removed from ankle IK. Concrete tasks in `ai/tasks/leg-hierarchical-solver-design.md` § «Slice 3 — knee-node controller»: `solveLegFromKneeTarget` helper, `KneeController` 3D component, `RigService.applyLegFromKneeTarget` plumbing, tests, UI handle.
