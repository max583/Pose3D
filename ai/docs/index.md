# AI Docs Index

| File | Description |
| --- | --- |
| `feature-task-template.md` | Template for non-Lite feature task briefs. |
| `r3f-smoke-manual-checklist.md` | Manual smoke/regression checklist for React Three Fiber viewport changes. |
| `browser-use-poseflow-3d-calibration.md` | Browser Use calibration notes for PoseFlow 3D viewport inspection. |
| `playwright-poseflow-3d-smoke.md` | Playwright 3D smoke and mouse-control documentation. |
| `controller-debug-protocol.md` | Process for complex controller/gizmo/IK fixes: model first, reference poses, focused tests, one-slice verification. |
| `collaboration-roles.md` | Universal role-based workflow for complex PoseFlow tasks, with profiles for mechanics, UI, import, R&D, and release work. |
| `design-doll-controls-poseflow-spec.md` | Domain/UX reference for DesignDoll-style controls; check current code first. |
| `source-modules.md` | Key source modules reference extracted from `CLAUDE.md` / `AGENTS.md`. |
| `unit-testing.md` | Unit-test rules extracted from `CLAUDE.md` / `AGENTS.md`. |
| `archive/index.md` | Archived/stale docs no longer used as active guidance. |
| `README.md` | Compatibility pointer to this index. |
