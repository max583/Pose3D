...
# Session note 2026-05-05 / Natural arm limits and Electron EPIPE

Natural arm limits were implemented as the next small vertical slice.

- Added `poseflow/src/lib/rig/armLimits.ts`.
- Wrist IK rejects candidate chains that overfold the elbow past 150 degrees.
- Upper-arm direction is checked in the local mannequin/shoulder frame, so limits follow root rotation.
- Axial twist measurement is available for upper arm bones `2-3` / `5-6` and forearm bones `3-4` / `6-7`.
- `RigService.applyElbowTwist` rejects candidate elbow positions outside upper-arm direction limits.
- Manual checks accepted wrist IK stop-at-limit and the next arm-limit pass.

Electron main-process logging was hardened against closed stdout/stderr pipes. The JavaScript error dialog with `EPIPE: broken pipe, write` should no longer crash the app; file logging remains active.

Technical checks:

- `npm test` passed: 252 unit tests.
- Focused arm/shoulder tests passed.
- `npm run typecheck` passed.
- `npm run build` passed and updated `dist-electron/main.js`.
- `npm run smoke:browser -- e2e/poseflow-3d-smoke.spec.ts e2e/poseflow-gizmo-regression.spec.ts` passed: 5 Playwright tests.

# Session note 2026-05-05 / Leg hip-limit interaction

Fixed the leg case where ankle IK could feel stuck after knee twist even though the visible pose looked natural. The root cause was an interaction between preserved knee twist plane and hip direction limits.

- `RigService.applyKneeTwist` now stops before the candidate knee position moves the thigh outside existing hip direction limits.
- `solveLegIKWithinLimits` now makes one safe re-limit pass for reachable ankle targets when the preserved knee twist plane conflicts with hip limits.
- Impossible ankle targets still keep the last valid pose.
- Follow-up: knee twist also blocks knee-plane rotation beyond `±90°` from the drag-start plane, preventing full turns around the `HIP -> ANKLE` gizmo axis.
- Second follow-up: leg IK candidates now reject backward knee flexion directly; preserved-twist ankle IK candidates are validated before being returned.
...
Manual check still needed on the reported leg pose in the viewport.

# Session note 2026-05-05 / Leg model refactor needed

Manual validation after the focused leg-limit fixes found a deeper model problem:

- Dragging the ankle forward/up can rotate the kneecap backward.
- Natural knee flexion, where the lower leg folds backward/up toward the body, is blocked or confused with an invalid "knee backward" state.
- The lower leg should not freely rotate around its own long axis relative to the knee; tibia axial twist must be strongly limited.

Research conclusion:

- The next leg pass should not be another sign flip.
- The leg solver needs an explicit anatomical model:
  - `kneeAnterior` / patella-facing direction;
  - true `kneeFlexion` separate from IK radial sign;
  - tightly limited `tibiaAxialTwist`;
  - ankle IK plane selection that keeps the patella anterior.
- Reference ranges recorded in `ai/tasks/leg-limits-refinement-analysis.md`: hip ROM, knee flexion/extension, ankle/subtalar ROM.

Accepted next plan:

1. Add pure leg-frame and knee-anatomy helpers in `legLimits.ts` / `legIK.ts`.
2. Rework ankle IK to choose/preserve the knee plane only while the patella stays anterior.
3. Raise knee flexion toward a natural first-pass range, likely 130°.
4. Keep hyperextension near 0° initially.
5. Limit tibia axial twist tightly, likely ±10..15°.
...
Current working tree note: several fixes are implemented and verified but not committed, including natural arm limits, Electron EPIPE hardening, leg limit focused fixes, and documentation. Latest full unit run: `npm test` passed with 263 tests; latest `npm run typecheck` passed.

# Next session priority / Documentation cleanup first

Before continuing leg-model work, the first task is documentation and repository-structure cleanup for context economy.

Required order:

1. Review active docs: `STATUS.md`, `PLAN.md`, `ai/tasks/`, `ai/docs/`, `plans/`, and related README files.
2. Move completed plans, obsolete task briefs, and long historical notes into archive files/directories.
3. Keep active docs short: current status, immediate leg-model refactor plan, working rules, and links to archive.
4. Reassess the documentation directory structure critically so future sessions load fewer files.
5. Update navigation/README entries after moving content.
6. Only then continue the anatomical leg-model refactor.

Reason: current context is getting saturated by accumulated history; cleaning it up should be treated as a first-class engineering task, not optional housekeeping.
